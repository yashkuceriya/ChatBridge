...
function GameOverModal({
  ...
  const iconColors = { win: "text-yellow-400", loss: "text-red-400", draw: "text-zinc-400" };
  const bgColors = {
  ...
  return (
    <div className="absolute inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm rounded-md">
      <div className={`bg-gradient-to-b ${bgColors[result]} border border-zinc-700 rounded-xl p-5 mx-4 w-full max-w-[260px] text-center shadow-2xl transition-all duration-300 ${visible ? "scale-100 opacity-100" : "scale-75 opacity-0"}`}>
        ...
        <div className="flex gap-2">
          ...
          <button onClick={onNewOpponent} className="flex-1 flex items-center justify-center gap-1.5 rounded-lg bg-zinc-700 hover:bg-zinc-600 py-2.5 text-xs font-semibold text-zinc-200 transition-colors">
            New Opponent
          </button>
        </div>
      </div>
  ...
  return (
    <div className="absolute inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm rounded-md">
      <div className="bg-zinc-900 border border-zinc-700 rounded-xl p-5 mx-4 w-full max-w-[240px] text-center shadow-2xl">
        ...
        <div className="flex gap-2">
          <button onClick={onCancel} className="flex-1 rounded-lg bg-zinc-700 hover:bg-zinc-600 py-2 text-xs font-semibold text-zinc-200 transition-colors">Cancel</button>
          <button onClick={onConfirm} className="flex-1 rounded-lg bg-red-600 hover:bg-red-500 py-2 text-xs font-semibold text-white transition-colors">Resign</button>
        </div>
      </div>
    </div>
  );
    ...
    <div className="flex flex-col h-full">
      <div className="px-4 py-3 border-b border-zinc-800">
        ...
        <div className="flex items-center gap-2 mt-1.5">
          ...
          <span className={`inline-flex items-center gap-1 text-xs font-semibold px-2 py-0.5 rounded-full ${playerColor === "white" ? "bg-zinc-100 text-zinc-900" : "bg-zinc-700 text-zinc-100"}`}>
          ...
          {BOT_PROFILES.map((bot, idx) => {
            ...
            return (
              <button
                ...
                className={`relative flex flex-col items-center rounded-xl border p-3 text-center transition-all duration-200 hover:scale-[1.03] ${isSelected ? "border-green-500 bg-green-900/20 shadow-[0_0_15px_rgba(34,197,94,0.3)]" : "border-zinc-800 bg-zinc-900/50 hover:border-zinc-600 hover:bg-zinc-900"}`}
              >
                <div className="h-12 w-12 rounded-full bg-zinc-800 flex items-center justify-center text-2xl mb-2">
                  {bot.avatar}
                </div>
                <span className="text-xs font-bold text-zinc-200 mb-0.5">{bot.name}</span>
                <span className={`text-[10px] font-mono px-2 py-0.5 rounded-full mb-1.5 ${ratingColor}`}>{bot.rating}</span>
                <div className="flex gap-0.5 mb-1.5">
                  {Array.from({ length: 5 }, (_, i) => (
                    <Star key={i} className={`h-2.5 w-2.5 ${i < difficulty ? "text-yellow-400 fill-yellow-400" : "text-zinc-700"}`} />
                  ))}
                </div>
...
export default function ChessApp() {
  ...
  useEffect(() => {
    ...
    // Step 1: Signal APP_READY
    sdk.ready(["chess"]);
    console.log("[Chess] APP_READY sent");

    // Step 2: Listen for SESSION_INIT
    const unsubSession = sdk.onSessionInit((sessionId, token, config) => {
      console.log("[Chess] SESSION_INIT received", { sessionId });
      setSessionActive(true);
      // If config includes a color preference, apply it
      if (config.color) {
        setPlayerColor((config.color as string).toLowerCase() === "black" ? "black" : "white");
      }
    });

    // Step 3: Listen for TOOL_INVOKE
    const unsubTool = sdk.onToolInvoke((toolName, args) => {
      console.log("[Chess] TOOL_INVOKE received", { toolName, args });

      switch (toolName) {
        case "chess_start_game": {
          const color = (args.color as string)?.toLowerCase() === "black" ? "black" : "white";
          setPlayerColor(color);
          startGameFromToolRef.current?.(color, args.botName as string | undefined);
          break;
        }
        case "chess_make_move": {
          handleMakeMoveFromToolRef.current?.(args.move as string);
          break;
        }
        case "chess_get_game_status": {
          handleGetGameStatusRef.current?.();
          break;
        }
        case "chess_get_hint": {
        ...
        case "chess_resign": {
          handleResignFromToolRef.current?.();
          break;
        }
      }
    });
  ...
  // Refs for tool handlers that need current state
  const startGameFromToolRef = useRef<((color: "white" | "black", botName?: string) => void) | null>(null);
  const handleMakeMoveFromToolRef = useRef<((move: string) => void) | null>(null);
  const handleGetGameStatusRef = useRef<(() => void) | null>(null);
  const handleHintRef = useRef<(() => void) | null>(null);
  const handleResignFromToolRef = useRef<(() => void) | null>(null);
  ...
  const emitComplete = useCallback((result: Record<string, unknown>) => {
    sdkRef.current?.complete(result);
    console.log("[Chess] APP_COMPLETE sent", result);
  }, []);

  const emitError = useCallback((message: string, code: string = "CHESS_ERROR") => {
    sdkRef.current?.error(message, code);
    console.log("[Chess] APP_ERROR sent", { message, code });
  }, []);
  ...
  const startGame = useCallback((bot: BotProfile, color?: "white" | "black") => {
    const actualColor = color || playerColor;
    const actualComputerSide = actualColor === "white" ? "b" : "w";
    setSelectedBot(bot);
  ...
  // --- Tool handler refs (use current state) ---

  startGameFromToolRef.current = (color: "white" | "black", botName?: string) => {
    setPlayerColor(color);
    const bot = botName
      ? BOT_PROFILES.find(b => b.name.toLowerCase() === botName.toLowerCase()) || BOT_PROFILES[1]
      : selectedBot || BOT_PROFILES[1];
    startGame(bot, color);
  };

  handleHintRef.current = () => {
    try {
      const hint = findBestMove(game.fen(), 4, 0);
      if (hint) {
        sdkRef.current?.updateState({
          fen: game.fen(),
        ...
        emitError("No hint available for this position", "NO_HINT");
      }
    } catch (e) {
      emitError("Failed to compute hint", "HINT_ERROR");
    }
  };

  handleMakeMoveFromToolRef.current = (moveStr: string) => {
    if (gameOver) {
      emitError("Game is already over", "GAME_OVER");
      return;
    }
    if (!moveStr) {
      emitError("No move provided", "INVALID_MOVE");
      return;
    }
    try {
      const next = new Chess(game.fen());
      const result = next.move(moveStr);
      if (!result) {
        emitError(`Invalid move: ${moveStr}`, "INVALID_MOVE");
        return;
      }
      setGame(next);
      ...
      emitError(`Could not execute move: ${moveStr}`, "MOVE_ERROR");
    }
  };
  ...
  handleGetGameStatusRef.current = () => {
    ...
    const materialAdv = getMaterialAdv(captured);
    const ev = evaluatePosition(game.fen());
    ...
    sdkRef.current?.updateState({
      ...
      evaluation: ev,
      capturedByWhite: captured.white.map(p => PIECE_UNICODE[p] || p),
      capturedByBlack: captured.black.map(p => PIECE_UNICODE[p] || p),
    });
  };
  ...
  const handlePlayAgain = () => { if (selectedBot) startGame(selectedBot); };
  const handleResign = () => { if (!gameOver) setShowResignConfirm(true); };
  const confirmResign = () => {
    setShowResignConfirm(false);
    setResigned(true);
    setGameOver(true);
  ...
  const PlayerBar = ({ color, isTop }: { color: "white" | "black"; isTop: boolean }) => {
    ...
    return (
      <div className={`flex items-center justify-between px-2 py-1.5 rounded-lg transition-colors ${active ? "bg-zinc-800" : "bg-zinc-900/50"}`}>
        <div className="flex items-center gap-2">
          <div className={`h-7 w-7 rounded-full flex items-center justify-center text-sm ${isComp ? "bg-zinc-700" : "bg-green-700 text-white"}`}>
  ...
  // --- Game screen ---
  return (
    <div className="relative flex flex-col h-full min-h-screen bg-zinc-950">
      <div className="flex items-center justify-between border-b border-zinc-800 px-4 py-2.5">
        <div className="flex items-center gap-2">
          <span className="text-lg">&#9823;</span>
          <span className="text-sm font-semibold text-zinc-100">Chess</span>
            ...
            {moveHistory.length === 0
              ...
              : moveHistory.map((pair, i) => {
                  ...
                  return (
                    <div
                      key={i}
                      className={`flex items-center text-xs py-0.5 px-2 rounded transition-colors ${isLastPair ? "bg-zinc-800/70" : "hover:bg-zinc-800/30"}`}
                    >
                      <span className="w-6 text-zinc-600 font-mono text-[10px]">{i + 1}.</span>
                      <button
                        onClick={() => handleMoveClick(i, false)}
                        className={`w-16 text-left cursor-pointer hover:bg-zinc-700/50 px-1 rounded transition-colors ${viewingMoveIndex === i * 2 ? "bg-green-800/40 rounded" : ""}`}
                      >
                        {annotateMove(pair.white)}
                      </button>
                      <button
                        onClick={() => pair.black ? handleMoveClick(i, true) : undefined}
                        className={`w-16 text-left cursor-pointer hover:bg-zinc-700/50 px-1 rounded transition-colors ${viewingMoveIndex === i * 2 + 1 ? "bg-green-800/40 rounded" : ""}`}
                      >
